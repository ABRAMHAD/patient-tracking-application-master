...
class ListPatientDetailsComponent extends Component {
    ...
    editPatient(pId){
        this.props.history.push(`/add-patient/${pId}`);
    }

    deletePatient(pId){
        AdminServices.deletePatient(pId).then( res => {
            this.setState({patient: this.state.patient.filter(patient => patient.pId !== pId)});
        });
    }

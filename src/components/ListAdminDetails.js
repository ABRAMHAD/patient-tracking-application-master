...
class ListAdminDetailsComponent extends Component {
    ...
    deleteAdmin(adminId){
        AdminServices.deleteAdmin(adminId).then( res => {
            this.setState({admin: this.state.admin.filter(admin => admin.adminId !== adminId)});
        });
    }
                                ...
                                this.state.admin.map(
                                    admin =>
                                        <tr key={admin.adminId}>
                                            ...
                                            <td><button style={{ marginLeft: "10px" }} onClick={() => this.deleteAdmin(admin.adminId)} className="btn btn-danger">Delete</button></td>
